...
public class AppVoUtil {
	...
	/*
	 * TelephonyManager类提供访问与手机通讯相关的状态和信息的get方法
	 * 其中包括手机SIM的状态和信息、电信网络的状态及手机用户的信息
	 *
	 * TelephonyManager类的对象可以通过Context.getSystemService
     * (Context.TELEPHONY_SERVICE)方法来获得，需要注意的是有些通讯信息
     * 的获取对应用程序的权限有一定的限制，在开发的时候需要为其添加相应的权限
	 */



		@SuppressWarnings("static-access")
		public static String getAppVoInfo(Context context)
		{
	     ...
	     */
				...
				for(NeighboringCellInfo info:infos){
						...
						int cid=info.getCid();
						//获取邻居小区LAC，LAC: 位置区域码。为了确定移动台的位置，
						//每个GSM/PLMN的覆盖区都被划分成许多位置区，LAC则用于标识不同的位置区。
						info.getLac();
						info.getNetworkType();
						info.getPsc();
						//获取邻居小区信号强度

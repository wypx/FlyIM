...
{
		/*
		 我们可以通过File file = (File) getItem(position);拿到Item项的文件，
		 如果是在第一个并且不再根目录我们就把第一个也就是parentFile显示为：“返回上一级”，下一行的都隐藏掉。
					 ...
					 如果这个文件是一个文件夹，就把文件的大小显示为“文件夹”，类型和修改时间隐藏掉。
						...
						通过viewHolder.data.setText(new SimpleDateFormat("yyyy/MM/dd HH:mm").format(file.lastModified()));
						设置文件的最近修改时间。
